...
function Button({ children, variant = "default", className = "", ...props }) {
  const baseStyle = "px-3 py-1 sm:px-4 sm:py-2 rounded font-medium text-xs sm:text-sm w-full sm:w-auto";
  const variantStyles = {
...
function TabsList({ children }) {
  return <div className="flex flex-wrap gap-2 sm:gap-4 border-b border-gray-300 dark:border-gray-600">{children}</div>;
}

function TabsTrigger({ value, activeTab, setActiveTab, children }) {
  return (
    <button
      className={`px-3 sm:px-4 py-1 sm:py-2 text-xs sm:text-sm font-medium sm:w-auto text-center ${activeTab === value ? "border-b-2 border-blue-600 dark:border-blue-400" : "text-gray-600 dark:text-gray-300"}`}
...
function TabsContent({ value, activeTab, children }) {
  return activeTab === value ? <div className="p-3 sm:p-4 text-gray-800 dark:text-gray-200 text-xs sm:text-sm">{children}</div> : null;
}
...
export default function ProfilePage() {
  ...
  return (
    <div className=" mt-[57px] max-w-4xl min-h-screen mx-auto p-3 sm:p-4 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-lg shadow-md w-full">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 mt-[-30px] sm:mt-[-40px] p-3 sm:p-4 text-center sm:text-left">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300 text-xs sm:text-sm">Web Developer | Content Creator</p>
      ...
      <Tabs>
        <TabsList>
          ...
          <TabsTrigger value="photos" activeTab={activeTab} setActiveTab={setActiveTab}>
            Photos
          </TabsTrigger>
          <TabsTrigger value="videos" activeTab={activeTab} setActiveTab={setActiveTab}>
            Videos
          </TabsTrigger>
        </TabsList>
